[https://l1siki.github.io/english101/](https://l1siki.github.io/english101/)

# 30日で英語ユーザーになる方法
1日10フレーズ x 30日 = 300フレーズ。日常会話の80%をカバーできる量。英語思考ができる。

by りしき  
英語のみで生活してます。日本人の大学生向けに書きました。（単語力4000以上想定、日常会話に十分な単語力で単語学習必要なし。）  
具体的にはお世話になっている教授の生徒さん（認知科学知識あり想定）向けなので、一般には少しわかりづらい表現（システム１等）を使用しています。（わからなかったらググってください。）

---

### 一つのことだけに集中する。

人が集中できることは、一度に一つだけです。
英語を上達させたいなら、ほかのことはいったん忘れてください。
一か月だけ、英語のことだけを考えて生活してください。
そして、英語の「何に」集中するのかも決めてください。なんでもやろうとすると、結局一つもうまくいきません。
すべての雑音を排除して、英語頻出フレーズの反復スピーキング練習だけに集中する一か月を過ごせば、そのあとは自然に英語を使えるようになります。

---

### なぜアメリカ人は英語が得意なのか？
* それは、彼らが1日16時間英語を使っているからです。

なぜ日本は英語が苦手なのか？
* それは、私たちが1日16時間日本語を使っているからです。そして英語はほぼゼロです。

両方を同時に使うことはできません。もし英語がうまくなりたければ、日本語の時間を減らし、英語の時間を増やしましょう。

---

### 心の中の「独り言」を英語に変える

私たちは一日中、心の中で日本語を話しています。
それを英語に変えてください。それだけで、大量の時間を英語に充てられ、日本語の時間を減らせます。
「お腹減った」と感じたら、心の中で "I'm hungry" と言うのです。

---

### どうやって脳内の独り言を英語に変えるか

「よく使われるフレーズ」を練習しましょう。
[https://www.englishspeak.com/en/english-phrases](https://www.englishspeak.com/en/english-phrases)
ここからダウンロードし、すべての音声ファイルを音楽プレーヤーに入れてください。
聞いて、聞こえた通りに話す（シャドーイングする）のです。
英語はスポーツです。勉強ではありません。

---

### 300フレーズ話せれば、80%の日常会話をカバーできる。

一日10フレーズで30日間。

さらに500+フレーズまで練習すれば、自然な英語を話せるようになります。
例として挙げたサイトは1000フレーズ提供しているので、すべて網羅すれば、かなりの英語力を持ったと考えていいでしょう。

そして、フレーズがしゃべれるようになれば、その中の単語を入れ替えるだけで、ほとんどの状況に対応できるようになります。

---

### 英語を「システム１」で処理する。

あなたが英語を使わない理由は、カーネマンのいう「システム２」を使って大量のエネルギーを消費しているからです。
英語をあなたの「システム１」で処理できるように体にしみこませれば、楽に英語を扱えるようになります。  
チェスのグランドマスターがシステム１で複雑な戦況を読むように、訓練をすればだれでも英語をシステム１で処理できるようになります。

---

### 「スピーキング」だけが重要。

スピーキング（話す）、リスニング（聞く）、ライティング（書く）、リーディング（読む）。
日本の英語教育では、これらは同列に扱われています。

しかし、最も重要なのはスピーキングです。
1. 英語の使用時間が圧倒的に増えるからです。
2. 「脳内言語の切り替え」が言語学習において最も重要であり、それにはスピーキング（思考）が必要だからです。
3. 話せない言葉やフレーズは、聞き取るのも難しいからです。（そして聞き取れたとしても、慣れないフレーズはシステム２で処理しなければならないため、処理に時間とエネルギーを使います。実際のリスニングでは大量の情報が絶え間なく流れてくるため、システム２で処理を始めた時点でそこから後のインプットに割く計算能力と時間が枯渇します。つまり長いリスニングができない。）

リスニングは2番目に大切です。スピーキングができればできるようになります。
1. これも英語の使用時間を大幅に増やします。
2. 英語コンテンツを楽しんでれば勝手に上達します。

---

### 「英語が得意」になろうとしないでください

完璧を目指すのは時間とエネルギーの無駄です。
まずは1ヶ月で「下手な英語ユーザー」になってください。
スキルが完璧でなくても、毎日使っていれば勝手に上達します。

---

### 「英会話」は、いまは忘れてください。

英語思考ができるようになれば、英会話も自然とできるようになります。

英会話は非常に学習効率が悪いです。
まずは、英語で思考ができるようになること。
システム１で英語が処理できるようになることに集中しましょう。

英会話は、たまに自分を英語力を確認する程度に行いましょう。
機会費用の観点から、学習自体には使うべきではありません。
もし英会話をするときは、あなたが「一秒以内にコンスタントに的確に返答できるか」で評価してください。

---

### ライティングとリーディングは今は忘れなさい。後で楽になります。

上達したい気持ちはわかります。
でも、それは基礎を理解せずに応用問題を解くようなものです。

なぜか？
1. 人間の脳は、読み書きを効率的に行うようには適応していません。私たちにとってそれは難しいことなのです。
2. 言語とは「話す・聞く」ものです。読み書きは言語の「拡張システム」に過ぎません。
    * 例えば、日本語が文字体系を持ったのは4世紀のことです。文字ができる遥か昔から、人々は言葉を話していました。
3. 英語の使用時間を効果的に増やせません。アメリカ人でさえ、1日16時間読み書きすることは不可能です。でも、彼らは1日16時間考えたり（脳内で話したり）、聞いたりすることはできます。

---

### 文法と発音も今は忘れなさい。後で楽になります。

英語ユーザーになるのに文法がいらない理由：
1. 文法は「よく使うフレーズ」を練習するだけでカバーできます。
2. 実際にはほぼ役に立ちません。話す時（英語でも日本語でも）、通常1秒以内に反応しなければなりません。読むときも聞くときも一緒です。文法について考えている暇はないのです。書くときは役に立ちますが、文法修正アプリを使用したほうが正確です。アメリカ人でさえ完璧に文法は扱えないのです。
3. **カーネマンの言う「システム2」と同じで、脳のエネルギーを大量に消費するため、結局は使わなくなります。**

英語ユーザーになるのに発音がいらない理由：
1. ヨーロッパのリーダーたち（イェンス・ストルテンベルグなど）も発音は酷いものですが、非常に尊敬されています。
2. 練習がつまらないからです。

---

### 文法は全部Grammarlyに任せればよし（Grammarlyユーザーの68%はネイティブ）。

[Grammarly User Survey Analysis](https://www.grammarly.com/press/research/docs/grammarlystudentsurvey-121018133119-phpapp01.pdf)
> There were significantly more native English speakers (68%) than nonnative speakers (32%).

もしかしたらあなたはツールに頼ることがかっこ悪いと思っているかもしれません。
ネイティブはそんなこと気にしません。
そして、ネイティブは正しい文法を知らないのでツールを積極的に使います。

ネイティブが文法を知らない理由は、文法は「本当の英語」ではないためです。
文法とは「きちんとした文を書くためのルール」です。
そしてきちんとした文を書くときはネイティブはツールに頼ります。

> Students: Make up roughly 35% of the user base.
Professionals: The fastest-growing segment, now heavily targeted by their "Grammarly Business" and "Grammarly Go" (AI) initiatives.

Grammarlyユーザーのほとんどはプロフェッショナルです。
プロであるほどツールに頼ります。逐一、文法を確認してる時間がないので。

ちなみにきちんとしすぎた文法はSNS上ではかっこわるいので（Twitterで役所みたいな日本語使う人がかっこ悪いのと同じ。）、
少しくらい文法きたないほうがむしろネイティブっぽく映ります。ただし、意味の伝わる崩し方で。

もちろんフォーマルな場ではgrammarly(あるいはAI)等を用いてきちんとした文法を使いましょう。

---

### 自信をもってフォーマルな英語を書く方法

必要なのは3つです。
1. 最低限の英語スキル（スピーキング）
2. Grammarly
3. Google Translate 英>日
-  AI(任意。時間短縮、そしてより自然で効率的な英語を出力させる。)

手順:
1. 最低限の英語スキルを使い、英語で文を書く。（AIを利用するならこの段階）
これは重要です。なぜなら、英語で自然な表現と日本語で自然な表現は全く違うためです。翻訳ではこの溝は絶対に埋まりません。
2. Grammarlyで文法を修正する
3. Google Translate 英>日 であなたの書いた英文が正しい日本語で出力されることを確認する。
あなたの想定していた出力でなければ、英文の単語や構成を変える。
4. フォーマルな英文の完成。

日>英 翻訳は絶対に使わないでください。なぜなら、あなたには「正しい英語」が判断できないからです。
あなたが判断できるのは「正しい日本語」だけです。
だから、日本語を出力できるシステムだけを最終確認には使用してください。

これはAIの時代になっても同じです。結局、実際にあなたの意図した文章になっているかどうかは、あなたにしかわかりません。
AIは非常に便利(執筆時間を大幅に短縮できる)ですが、最終確認はあなたにしかできません。

---

### 発音について知るべきことは一つだけ。

音節（シラブル: syllable）です。
例えば、イエス(Yes)は日本人の感覚だと3音節ですが、英語では1音節です。つまり一音として発音します。
これを意識するだけでもリスニングとスピーキングが格段に上達します。
* [https://syllables.flap.tv/](https://syllables.flap.tv/)

上記サイトに適当な英文を入力すると、シラブルで分割してくれます。  
シラブルを意識して英語を喋りましょう。聞く時にもシラブルを意識すると聞き取りやすいです。  
例えば「Take it」は実際には"Ta keit"と発音したほうが自然です。先ほど話したように筆記体系というのは後付けシステムで、単語間のスペースは実際の英語には存在しません。「Take it」は「Takeit」となり、2音節の1単語のようにふるまいます。  

---

### シラブルは発音よりも重要。

シラブルは音ですが、それを大きく超えた重要性を持ちます。
文の総シラブルが少ないものほど、自然で、理解が容易で、伝わりやすく、記憶に残りやすい文になります。
たとえば、"計算をした" は "I did the math" が自然な表現です。 "I did the calculation" はあまり使われません。シラブルが長すぎるためです(4 vs. 7)。
それだけでなく、I did the math は I calculated よりも短い総シラブル (4 vs. 5) を持ちます。だからこの表現はよく使われるのです。
自然な、伝わる英語を身に着けたいときは、シラブルは非常に重要です。

- 4 シラブル: I did the math
- 7 シラブル: I did the calculation
- 5 シラブル: I calculated 

---
### 🎵 シラブルを意識して英語の歌を歌ってみよう

シラブル（音節）を意識して歌うことは、英語特有のリズムを学ぶのに最適です。

**準備：Genius.comの歌詞をシラブル化する**
1. **Userscript Managerを導入**
   - [Chrome: Tampermonkey](https://chrome.google.com/webstore/detail/tampermonkey/dhdgffkkebhmkfjojejmpbldmpobfkfo)
   - [Firefox: Tampermonkey](https://addons.mozilla.org/en-US/firefox/addon/tampermonkey/)
2. **スクリプトをインストール(リンク先はこのrepoのルートにあるuser.jsファイル)**
   - [歌詞サイトGenius.comのみ](https://github.com/l1siki/english101/raw/refs/heads/main/syllabify-genius.user.js)
   - [すべてのサイト(Tampermonkeyで有効無効を切り替えてください。) ](https://github.com/l1siki/english101/raw/refs/heads/main/syllabify-everything.user.js)

> **「·」がシラブルの分割点です。**
> 例：**"real" (re·al)** は2音、**"this"** は1音。
> このリズムを意識するだけで、発音が劇的にネイティブに近づきます。
> 日本語で表現すると「り·あぅ」と「でぃㇲ」です。「L」はたいてい小さい「ぅ」と発音します。Thisは一息で、最後のsはほとんど発音しない小さなㇲだと思って発音してみてください。

**1. Shake it Off – Taylor Swift**
[YouTube](https://www.youtube.com/watch?v=nfWlot6h_JM) / [歌詞](https://genius.com/Taylor-swift-shake-it-off-lyrics)

これは究極の「自然」な英語の曲です。
ネイティブの会話のほとんどは、このように1シラブルの単語（Monosyllables）の連続です。
単語をつなげて、パーカッションのように歌うのがコツです。

**2. Bohemian Rhapsody – Queen**
[YouTube](https://www.youtube.com/watch?v=fJ9rUzIMcZQ) / [歌詞](https://genius.com/Queen-bohemian-rhapsody-lyrics)

この曲は多くの複数シラブル単語を使います。"reality"(re·al·i·ty)は4音で構成されています。
(ネイティブは複数シラブル単語を避けます。realityとしゃべる間に単シラブル単語を4つ言えるので、realityは非効率な単語だからです。文字上は短いのでインターネット上ではよく見かけるかもしれません。)

**おまけ：オフライン用Pythonスクリプト**
大量のテキストをシラブル化したい人向け。
[Download Script](https://github.com/l1siki/english101/raw/refs/heads/main/syllabify_all_txt_incl_subfolders.py)

---
### アメリカ人が楽しんでいるものを楽しむ

例えば、Redditに行ってみましょう。これはアメリカ発のSNSです。
* [https://www.reddit.com/r/all/](https://www.reddit.com/r/all/)
英語ですが、人気投稿の多くはシンプルな画像か、短い文章だけです。
英語がわからなくても楽しめます。そして楽しんでいれば好きになり、自然と英語がわかるようになります。
YouTubeを見たり、オーディオブックを聞いたり、アメリカ人がすることを何でもやってみましょう。

* Googleが高品質なAIを無料で提供しています。開発者向けですが誰でも使えます。 [https://aistudio.google.com/prompts/new_chat](https://aistudio.google.com/prompts/new_chat)
    * そこで英語で会話できます。次に何か検索する時は、そのAIに英語で聞いてみましょう。
    * AIを英語で言い負かす（例えば、論理矛盾の指摘）ことができるようになれば、英語で議論ができる素地があなたに備わったと思っていいでしょう。
        * 例として、常識外れな主張をしてAIを英語で納得させてみてください。（たとえば「ハンニバルはすごくない」等）

---

### 日本語を生活から排除する（できるかぎり）

物理的に日本語が目に入ったら、それを英語に置き換えられないか考えてください。
YouTubeは英語で見る。
アニメは英語音声で見る。ゲームは英語でプレイする。漫画は英語で読む。音楽は英語のものだけ聞く。OS（スマホやPCの設定）を英語に変える。
今日からアメリカ人になったつもりで娯楽を消費してください。

日本語や「日本語を思い起こさせるもの」（プライミング効果）はできる限り生活から排除することで、英語思考の時間を自然に増やせます。（自然に日本語思考を減らす。）

日本語で説明されてる「英語学習コンテンツ」などは英語学習の障害です。  
「語学学校」も同様で、英語学習の障害です。  

---

### 「プライミング」を利用するため、英語を思い起こすものを生活に取り入れる。

例えば、英語の本をデスクの上に置くだけで、それが目に入るたびにあなたは英語のことを思い出します。
これは「プライミング効果」といわれ、「連想させること」といった意味を持ちます。
たとえその本を読まなくても、ただ視界に入るだけで、その本はあなたの英語学習を少し楽にします。

この認知効果を利用すれば、脳の認知バイアスを利用して、自然に英語時間を増やせます。
もっとも強力なものは、後述する方法を使って「一日中英語音声を聞いている環境」で常時プライミングします。


---

### 「プライミング」を利用するため、日本語を生活から隠す。

日本語、あるいはあなたに何か日本語を思い出させるものは英語時間を奪います。
生活の中で目に入る日本語、あるいは日本語を呼び起こさせるものは隠してください。

---

### さあ、今何をすべきか？

1. **安い（千円程度、画面不要）ポータブルオーディオプレーヤーを買ってください。** 充電切れ（ダウンタイム）を防ぐために2台持っておきたいところです。
    * これは重要です。なぜなら**スマートフォンは「気が散るマシン」だからです。** 言語学習に特化したシンプルな道具が必要です。
    * そして「プライミング効果」。英語学習専用のデバイスは、それが視界に入った瞬間に「英語学習」を思い出させます。これはスマートフォンには持てない機能です。あなたはスマートフォンを見たとき、SNSや友達、あるいはゲームや予定のことを思い出します。それらはすべて英語学習の障害です。英語以外のことをあなたに思い起こす物は、視界から排除すると学習効率が上がります。
3. 「よく使われるフレーズ」をダウンロードして、オーディオプレーヤーに入れてください。（例：[https://www.englishspeak.com/en/english-phrases](https://www.englishspeak.com/en/english-phrases)）
4. 朝起きたら、すぐに聞き始めてください。心の中で聞いたフレーズを繰り返すか、実際に声に出しましょう（起きたばかりはのどを痛めやすいので注意）。
5. ポケットに入れて、できるだけ英語に触れ続けましょう。 1フレーズ最低100回、口に出しましょう。

---

### おまけ：自然なフレーズを手に入れる別の方法

1. お気に入りの海外ドラマを見つける。（あるいは好きなアニメの英語吹き替え等）
2. 動画と字幕ファイルをダウンロードする。
3. 動画から音声を抽出する。
4. Python等を使い、字幕ファイルのタイムスタンプに合わせて音声の一部（セリフ）を切り出す。
5. これで、練習用のお気に入りドラマのフレーズ集の完成です。

あるいは、オーディオブックをAIで解析し、タイムススタンプと台詞を特定してから音声を抽出することもできます。

---

### おまけ: お金をかけてもいい人向けオーディオプレイヤー

骨伝導型オーディオプレイヤーが最も適しています。つけっぱなしにできるので。
1. 耳をふさがずに聞ける（人と会話ができる。外で使っても環境音が聞こえて危なくない。）
2. たいてい防水機能つき。ちょっとのシャワー中にも聞ける。（お風呂はお勧めしません、長時間の比較的高い温度の飽和湿度は苦手なようです。執筆者はShokzの高級モデルを二個壊しました。）
3. 耳にやさしい。疲れない。
  
集中したいときは耳栓と組み合わせるとノイズキャンセリングとは比べ物にならないほどの遮音性能を発揮できます。

商品例として（2025/12 調べ）
1. Shokz: OpenSwim Pro (¥18,000) ⭐おすすめ
2. Creative: Outlier Free Pro+ (¥8,000)

常時使用するためには、二個必要です。最初は一つだけ買って、気に入ったらもう一つ同じものを買いましょう。たいてい専用充電器なので、冗長性を持たせるため同モデルを買うことをお勧めします。充電器が壊れてもスペアを使えるように。    
単独再生できるものを選んでください。ワイヤレス骨伝導ヘッドフォンは選ばないでください。

---
